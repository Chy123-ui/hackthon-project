    ...
    <div>
      <div style={{ display: "flex", gap: 8, alignItems: "center", marginBottom: 8 }}>
        <select
          ...
          style={{
            ...
            color: "var(--text)", fontSize: 14, minWidth: 150,
          }}
        >
          {worlds.map((w) => (
        ...
        {fileLabels.map((f) => (
          <button key={f} className={`tab-btn ${activeFile === f ? "active" : ""}`}
            onClick={() => onSelectFile(f)} style={{ padding: "8px 16px", fontSize: 13 }}>
            {f}
          </button>
        ))}
      ...
      <div className="template-row">
        <div className="template-column">
          ...
          <textarea value={fileData} onChange={(e) => onFileDataChange(e.target.value)} style={{ minHeight: 300 }} />
        </div>
        <div className="template-column">
